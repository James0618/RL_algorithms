...
class DQN:
    def __init__(self, n_replay, n_state, n_action, device, load_param=False, learning_rate=0.005, gamma=0.95,
                 epsilon=0.05):
        # parameters init
    ...
    def learn(self):
        ...
        data_state = torch.from_numpy(data_array[:, 0:self.n_state]).float().to(device=self.device)
        data_action = torch.from_numpy(data_array[:, self.n_state:self.n_state+1]).long().to(device=self.device)
        data_reward = torch.from_numpy(data_array[:, self.n_state+1:self.n_state+2]).float().to(device=self.device)
        data_state_ = torch.from_numpy(data_array[:, -self.n_state:]).float().to(device=self.device)
